...
const Label = styled.label<{ $ishint: boolean }>`
  ...
  font-size: ${({ theme, $ishint }) =>
    $ishint ? theme.typography.sizes.hint : theme.typography.sizes.body};
  font-weight: 700;
  color: ${({ theme, $ishint }) =>
    $ishint ? theme.colors.textSecondary : theme.colors.textPrimary};
  text-align: left;
  display: ${({ $ishint }) =>
    $ishint ? "none" : "block"}; // Hide label if isLabelHint is true
...
const TextAreaField = styled.textarea<{ $haserror: boolean }>`
  ...
  min-height: calc(
    ${({ theme }) => theme.typography.sizes.body} * 15
  ); // Minimum 6 lines
  max-height: calc(
    ${({ theme }) => theme.typography.sizes.body} * 152
  ); // Maximum 72 lines

  &::placeholder {
    color: ${({ theme }) => theme.colors.textSecondary}; // Set hint color
    font-size: ${({ theme }) =>
      theme.typography.sizes.hint}; // Set hint font size
  }
