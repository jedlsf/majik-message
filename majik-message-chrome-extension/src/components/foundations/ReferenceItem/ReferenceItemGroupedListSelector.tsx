...
import { DotsThreeIcon, EraserIcon, Icon, PlusIcon } from "@phosphor-icons/react";
...
type AccessorFormat = "string" | "full-name" | "date" | "relative-time" | "time" | "address" | "bool-yes-no";
...
const filterItems = (
    ...
    return items
        .filter((opt) => !selected.some((sel) => String(sel.id) === String(opt[idKey])))
        .map((opt) => {
            const rawValue = resolveAccessor(opt, group.accessorKey);
            const label = formatAccessorValue(rawValue ?? opt[idKey], group.accessorFormat);

            // ---- handle subtext ----
            let subtext: string | undefined;
            if (group.extras?.subtext?.accessorKey) {
                const rawSub = resolveAccessor(opt, group.extras.subtext.accessorKey);
                subtext = formatAccessorValue(rawSub, group.extras.subtext.accessorFormat);
            }
            ...
            <MobileWrapper>
                ...
                <MobileDropdown
                    ...
                    onChange={(e) => {
                        ...
                        if (selectedId.startsWith("add-")) {
                            ...
                            const targetGroup = groupedOptions.find((g) => g.group === groupName);
                            targetGroup?.emptyActionButton?.();
                            return;
                        }

                        const found = groupedOptions.flatMap((g) =>
                            g.filtered.map((opt) => ({ group: g, opt }))
                ...
                >
                    <MobileOption value="" disabled>
                        {`Add New ${tooltip}` || "Select an option"}
                    </MobileOption>
