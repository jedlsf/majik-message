...
const DatePicker: React.FC<DatePickerProps> = ({
  ...
  useEffect(() => {
    if (selectedYear && selectedMonth && selectedDay) {
      ...
      if (minDate && pickedDate < new Date(minDate)) {
        setError(`Date cannot be earlier than ${new Date(minDate).toLocaleDateString()}`)
        return
      }

      if (maxDate && pickedDate > new Date(maxDate)) {
        setError(`Date cannot be later than ${new Date(maxDate).toLocaleDateString()}`)
        return
      }
  ...
  const generateMonthOptions = (): { value: number; name: string }[] => [
  ...
  const generateDayOptions = (): number[] => {
    ...
    if (selectedYear && selectedMonth) {
      ...
      for (let i = 1; i <= daysInMonth; i++) {
        const potentialDate = new Date(parseInt(selectedYear), parseInt(selectedMonth) - 1, i)
