...
const StyledDropdownMenuContent = styled(DropdownMenuContent)`
  ...
  scrollbar-color: ${({ theme }) => theme.colors.secondaryBackground} rgba(0, 0, 0, 0); /* Thumb and track colors */
`
  ...
  const normalizeToStringArray = (input: any): string[] => {
    ...
    if (typeof input === 'object' && !Array.isArray(input)) {
      ...
      if (isNumericEnum) {
        ...
        return values.filter((v) => typeof v === 'number').map((v) => String(v))
      } else {
    ...
    if (Array.isArray(input) && typeof input[0] === 'object' && input[0] !== null) {
      return input
            ...
            <DropdownMenuTrigger asChild>
              <CenteredTextWrapper>
                ...
                (typeof value === 'number' && !isNaN(Number(value))) ? (
                  ...
                  <UnsetOptionItem>{tooltip || 'Click here to change'}</UnsetOptionItem>
                )}
              </CenteredTextWrapper>
            </DropdownMenuTrigger>
                ...
                .map((item) => (
                  <DropdownMenuItem
                    ...
                    onSelect={() => {
                      ...
                      const parsedValue =
                        !isNaN(Number(item)) && normalizedOptions.includes(String(Number(item)))
                          ? Number(item)
                          : item
