...
const ErrorPlaceholder: React.FC<ErrorPlaceholderProps> = ({ children, path = '' }) => {
  const navigate = useNavigate()
  const handleGoBack = (): Promise<void> =>
    !!path && path.trim() !== ''
  ...
  return (
    <PlaceholderContainer>
      ...
      <Image src={'./favicon-256x256.png'} alt="Error Placeholder" />
      <ButtonColumn>
        <ButtonPrimaryConfirm onClick={handleGoBack}>Go Back</ButtonPrimaryConfirm>
        <ButtonPrimaryConfirm onClick={handleRefresh}>Refresh</ButtonPrimaryConfirm>
      </ButtonColumn>
    </PlaceholderContainer>
  )
}
