...
const InputBudget: React.FC<InputProps> = ({ onUpdate, currentValue = null }) => {
  ...
  const handleUpdateCurrency = (input: string): void => {
    const updatedDetails: BudgetRange = { ...inputData, currency: input || 'PHP' }

    onUpdate?.(updatedDetails)
  }

  const handleUpdateMinimum = (input: string): void => {
    const updatedDetails: BudgetRange = { ...inputData, minimum: parseFloat(input) || 0 }

    onUpdate?.(updatedDetails)
  }

  const handleUpdateMaximum = (input: string): void => {
    const updatedDetails: BudgetRange = { ...inputData, maximum: parseFloat(input) || 0 }
