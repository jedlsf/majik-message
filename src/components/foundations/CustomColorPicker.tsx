...
const CustomColorPicker: React.FC<CustomColorPickerPickerProps> = ({ currentValue, defaultColor = '#d6f500', max = 10, onUpdate }) => {
    const [pickedColors, setPickedColors] = useState<string[]>(currentValue || [defaultColor]);
    ...
    useEffect(() => {
        setPickedColors(currentValue || undefined);
    }, [currentValue]);
                    ...
                    <ListContainer>
                        {pickedColors.map((color, index) => (
                            <RowItemContainer key={index}>
                                <ColorBox color={color} onClick={() => editColor(index, color)} />
